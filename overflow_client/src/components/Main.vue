...
export default {
  ...
  methods: {
    ...
    like(val) {
      // ketika gw nge like, gw butuh cek ada gak isi dari val.like 
      // kalo ga ada, gw langsung push 
      // kalo ada, gw looping, cek ada ga email nya ? kalo ga ada, push lagi
      if (val.email == firebase.auth().currentUser.email) {
        alert('cannot like your own post');
          ...
          likes.once('value', (snapshot) => {
            ...
            if (alreadyVote) {
              alert('u already like this question')
              var dislikes = firebase.database().ref(`/question/${val.key}/dislike`);
              dislikes.once('value', (snapshot) => {
                let object = snapshot.val();
                ...
                // time to delete dislike content
                var dislikes = firebase.database().ref(`/question/${val.key}/dislike`);
                dislikes.once('value', (snapshot) => {
                  let object = snapshot.val();
                  let dislike_key = null
    ...
    dislike(val) {
      ...
      dislikes.once('value', (snapshot) => {
        ...
        if (object) {
          ...
          if (alreadyVote) {
            alert('u already dislike this question')
            var likes = firebase.database().ref(`/question/${val.key}/like`);
            likes.once('value', (snapshot) => {
              let object = snapshot.val();      
              let like_key = null
