<template>
    <div id="single">
        <div v-if="!isLoading">
            ...
            <h3>Answers : </h3>  
            <card-detail :usage="'answer'" v-for="(answer, index) in currentQuestion.answers" :key="index" :item="answer" @edit-answer="editAnswer($event)" class="mb-2" ></card-detail>
        </div>
...
export default {
    ...
    methods: {
        ...
        edit(){
            ...
            api.patch(`/answers/${this.answer.id}`, {body: this.answer.body}, {headers: {token: localStorage.token}})
                .then(({data})=> {
                    ...
                    this.getQuestion(this.$route.params.id).then((data)=> {
                        this.$swal.close()
                    })
                    this.formUse = 'create'
